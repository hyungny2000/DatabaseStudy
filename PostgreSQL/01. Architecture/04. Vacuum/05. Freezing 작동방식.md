...
2. **테이블 Age가 VACUUM_FREEZE_TABLE_AGE를 넘지 않아 Lazy Mode Vacuum동작**. ALL_VISIBLE=TRUE인 페이지가 SKIP_PAGES_THRESHOLD를 넘는지 체크 (FREEZE_NEEDED변수에 저장)
3. 해당 테이블의 Visibility Map에서 페이지 별 VM정보를 가져옴
4. 페이지의 ALL_VISIBLE=FALSE인지 체크
5. 페이지의 ALL_VISIBLE=FALSE이면 다음 두 가지 실행. 
   1. DEAD TUPLE 정리
   2. TUPLE의 AGE가 VACUUM_FREEZE_MIN_AGE를 초과하면 FREEZING 실시
6. 페이지의 ALL_VISIBLE이 TRUE인 경우 FREEZE_NEEDED 값이 TRUE이면, 튜플의 Age가 VACUUM_FREEZE_MIN_AGE를 초과할 때 FREEZE실시.
7. FREEZE_NEEDED 값이 TRUE이면 테이블의 Age도 VACUUM_FREEZE_MIN_AGE와 일치시킴
8. **테이블의 Age가 VACUUM_FREEZE_TABLE_AGE를 초과하므로 Eager Mode Vacuum 동작**
9. Visibility Map에서 페이지별 ALL_FROZEN 값을 가져온 다음. ALL_FROZEN=TRUE인 페이지에 대해 다음 실시
    1. DEAD TUPLE 정리
    2. TUPLE의 AGE가 VACUUM_FREEZE_MIN_AGE를 초과하면 FREEZING 실시
10. 테이블의 Age도 VACUUM_FREEZE_MIN_AGE와 일치시킴
