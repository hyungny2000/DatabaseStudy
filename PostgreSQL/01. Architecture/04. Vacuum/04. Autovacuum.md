...
| 파라미터 | 설명 | 기본값 |
| ---  | --- | --- |
 | track_counts | Update/Delete 카운트 여부 | on |
...
#### Autovacuum 동작방식
1. 계산된 임계치 이상으로 Update/Delete 변경이 발생했으면 autovacuum으로 dead tuple을 정리한다.
2. 대상 테이블의 Age가 vacuum_freeze_table_age를 초과하면, vacuum_freeze_min_age값을 기준으로 튜플을 Freezing한다.
