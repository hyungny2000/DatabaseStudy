...
#### Autovacuum 동작방식
```
IF((AUTOVACUUM == ON && TRACK_COUNTS == ON) || (AUTOVACUUM_FREEZE_MAX_AGE < AGE(TABLE))
  IF(UPDATE_COUNT(TABLE) > THRESHOLD(TABLE)
    REMOVE_DEAD_TUPLES(TABLE);
    IF(AGE(TABLE) > VACUUM_FREEZE_TABLE_AGE)
      FOREACH(PAGE IN TABLE)
        IF(VM_ALL_FROZEN(PAGE) == FALSE)
          FOREACH(TUPLE IN PAGE)
