...
- 테이블의 Age가 vacuum_freeze_table_age를 초과하면 Eager Mode, 아니면 Lazy Mode로 동작한다.
- 튜플의 Age가 vacuum_freeze_min_age를 초과하면 Freezing된다.
...
### Lazy Mode에서의 맹점
Insert만 되고 Update/Delete는 없는 로그성 테이블의 경우, dead tuple은 없으므로 ALL_VISIBLE=1이다. 하지만 Insert된 데이터들의 XMIN도 너무 오래되면 Freezing 과정을 거쳐야 하므로 ALL_FROZEN=0인 페이지를 Freezing하기 위해 Eager Mode가 필요하다.
