...
- 테이블 총 페이지 수가 32개 미만이어도 SKIP_PAGES_THRESHOLD 값

### 동작방식
```
FREEZE_NEEDED = SKIP_PAGES_THRESHOLD > COUNT_ALL_VISIBLE_TRUE(TABLE)

IF (AGE(TABLE) < VACUUM_FREEZE_TABLE_AGE) -- 1.
   FOREACH(PAGE IN VISIBILITY_MAP(TABLE)) -- 2.
      IF(ALL_VISIBLE(PAGE) == FALSE)
         PURGE_DEAD_TUPLES(PAGE);    -- 3.
         FREEZE(PAGE);           -- 3.
      ELSE
   ...
   END;
   IF(FREEZE_NEEDED == TRUE)
      AGE(TABLE) = VACUUM_FREEZE_MIN_AGE -- 5.
   END;
END;
```
1. TABLE의 AGE가 VACUUM_FREEZE_TABLE_AGE 파라미터 값 미만이면 LAZY MODE로 VACUUM을 실행한다.
2. Visibility Map에서 각 페이지의 VM정보를 호출
3. ALL_VISIBLE=FALSE이면 DEAD TUPLE을 정리하고, FREEZE를 실행한다.
4. ALL_VISIBLE=TRUE이면 DEAD TUPLE은 없는 상태임.
   1. 테이블의 ALL_VISIBLE=TRUE인 페이지의 개수가 SKIP_PAGES_THRESHOLD(32개) 이상인 경우 FREEZE를 생략한다.
   2. 테이블의 ALL_VISIBLE=TRUE인 페이지의 개수가 SKIP_PAGES_THRESHOLD 이하인 경우 FREEZE를 실시한다.
5. FREEZE를 실행했다면 테이블의 AGE를 VACUUM_FREEZE_MIN_AGE 파라미터와 일치시킨다.
