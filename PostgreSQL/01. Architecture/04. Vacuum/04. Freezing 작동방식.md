...
3. 테스트 테이블 생성 및 데이터 적재.
   ...
   create table test2 (col1 int primary key, col2 int);
   
   insert into test1 select generate_series(1,1000), generate_series(1,1000); -- 1000건 입력
   insert into test1 select generate_series(1,1000000), generate_series(1,1000000); -- 100만 건 입력

   select count(*) from pg_visibility('test1'); -- 5개 페이지
   select count(*) from pg_visibility('test2'); -- 4425개 페이지
   ```
...
4. test1 테이블을 update하는 
   ...
   --------------
           21478
   ```
   - test1테이블 truncate 이후 데이터 재적재, test2 테이블 vacuum full
   ```
...
6. 모니터링 쿼리 준비
   ...
   select * from pg_stat_all_tables where relname in ('test1','test2');
...
8. vacuum 직전 데이터베이스, 테이블, 튜플 나이 확인
   ```
   select * from pg_visibility('test1') limit 2;
