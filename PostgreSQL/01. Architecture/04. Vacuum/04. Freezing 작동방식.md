...
- ALL_VISIBLE=TRUE인 페이지가 연속으로 32개 이상일 때 Freeze작업을 Skip한다.
...
4. ALL_VISIBLE=TRUE이면 DEAD TUPLE은 없는 상태임.
   1. 테이블의 ALL_VISIBLE=TRUE인 페이지의 개수가 32개 이상인 경우 FREEZE를 생략한다.
   2. 테이블의 ALL_VISIBLE=TRUE인 페이지의 개수가 32개 이하인 경우 FREEZE를 실시한다.
