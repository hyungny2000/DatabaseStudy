...
### Lazy Mode 테스트
#### 테스트환경 세팅
1. 테스트 테이블 생성 및 데이터 적재.
   ```
   create table test1 (col1 int primary key);
   insert into test1 as select generate_series(1,1000);
   ```

2. pgbench 벤치마킹 준비 (트랜잭션ID 소모용도)
   ```
   nano =# create database pgbench;
   nano =# exit;

   [postgresql] pgbench -i -d pgbench
   ```
