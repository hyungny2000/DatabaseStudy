...
다른 DBMS와 달리 PostgreSQL에서는 Update/Delete시 Undo블록을 생성하는 대신 과거 버전의 튜플을 페이지에 그대로 남겨놓음으로써 MVCC를 구현했다. 하지만 **과거 버전 튜플은 재사용하지 못하므로, 계속 누적되어 디스크 공간을 낭비하는 Table Bloat이라는 문제점**이 있다. 과거 버전의 튜플이 영원히 필요하지는 않아서 언젠가 이들을 제거하고 공간을 비워둬야 하는데 이 개념을 Vacuum이라 한다. Vacuum이란 단어는 '진공청소기로 청소하다'라는 뜻이 있다.
2. Anti XID Wraparound      
PostgreSQL에서의 XID는 최대 값이 2^32(약 42억 9천만)정도이다. 최대치를 넘기고 나면 다시 3부터 시작하게 되는데, 이럴 경우 XID가 3인 트랜잭션의 입장에선 DB의 모든 데이터가 미래시점 데이터로 간주되어 모든 데이터를 잃게 되고 데이터 정합성이 깨진다. 이를 방지하기 위해 주기적으로 튜플의 XID를 Freezing해 주어야 한다. Frozen XID는 어떤 XID보다도 오래된 버전으로 간주되어 항상 볼 수 있다. autovacuum이나 vacuum freeze 수동 실행을 통해 XID Wraparound현상을 방지할 수 있다.
