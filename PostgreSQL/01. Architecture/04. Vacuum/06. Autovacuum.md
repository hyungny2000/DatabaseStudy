...
1. Vacuum 임계치
   ...
   vacuum threshold = autovacuum_vacuum_threshold
       + autovacuum_vacuum_scale_factor * pg_class.reltuples of the table;
   ```   
...
4. Vacuum analyze 임계치
   ...
   vacuum analyze threshold
     = autovacuum_analyze_threshold
        + autovacuum_analyze_scale_factor * pg_class.reltuples of the table
   ```

## AUTOVACUUM_FREEZE_MAX_AGE 파라미터
- autovacuum = off 상태에서도 테이블의 Age가 AUTOVACUUM_FREEZE_MAX_AGE를 넘기면 강제로 autovacuum을 하도록 하는 파라미터.
