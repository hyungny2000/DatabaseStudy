...
- 간혹 PostgreSQL에서는 **특이하게** SELECT 쿼리에도 트랜잭션 ID가 부여된다고 설명하는 글이 보이곤 하는데, 모든 DBMS는 SELECT쿼리에도 당연하게 트랜잭션ID가 부여된다. 이 트랜잭션에서 봐도 되는(Visible) 데이터와 보면 안되는(Invisible) 데이터를 판단하는데 트랜잭션ID가 사용되기 때문이다. 오라클도 SELECT 쿼리에 SCN을 부여하여 MVCC를 구현한다.
