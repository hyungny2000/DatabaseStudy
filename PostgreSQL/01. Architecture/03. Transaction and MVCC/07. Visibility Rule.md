...
END
```
1. XMIN 트랜잭션이 Frozen상태라는 건, XMIN트랜잭션 숫자가 아무리 커도 모든 트랜잭션에 Visible하다는 의미이다. (Vacuum에서 자세히 설명함) 참고로, Vacuum과정에서 DEAD TUPLE은 제거되므로, FROZEN TUPLE의 상태는 따로 체크할 필요가 없다. 
