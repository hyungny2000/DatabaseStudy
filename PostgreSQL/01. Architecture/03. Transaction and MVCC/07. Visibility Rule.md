...
8. XMAX 트랜잭션이 Commit되었다는 건, 이 튜플이 Update/Delete으로 인해 과거 버전이 된 튜플이라는 의미이다. 삭제가 되었다고는 하나 (Repeatable Read격리레벨에서) 트랜잭션 스냅샷에 의해 현재진행형 트랜잭션으로 간주되면 데이터를 볼 수 있어야 한다. 따라서 Visible이다.
