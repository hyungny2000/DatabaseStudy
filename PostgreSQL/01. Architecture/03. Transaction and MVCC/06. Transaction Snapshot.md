...
- Oracle의 경우, Undo Segment로부터 과거의 SCN버전 블록을 찾을 때까지 계속 로딩하는 방식으로 MVCC를 구현한다.
- PostgreSQL의 경우, Undo매커니즘은 없고 Update/Delete된 힙 튜플을 페이지에서 지우지 않고 그대로 두어 MVCC를 구현한다.
...
- Repeatable Read
  ...
  - 4개 트랜잭션이 커밋하여 실제로 종료된 상태이지만, 트랜잭션 스냅샷에서의 활성 트랜잭션 목록에는 있기 때문에 볼 수 없음.
- Serializable
