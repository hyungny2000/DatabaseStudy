...
- 동일한 데이터라도 각 트랜잭션의 격리레벨, 활성화여부에 따라 봐야 하는 데이터 버전이 다르다.
...
- 트랜잭션 시작 시 Undo블록을 확보한다.
- Update/Delete 직전 블록을 Undo블록으로 복사한 후에 원 블록을 변경한다. 해당 Undo블록이 현 블록의 과거버전인 셈이다.
...
#### Commit Log와 Transaction Slot의 차이점
트랜잭션 정보를 관리한다는 면에서 Commit Log와 Transaction Slot이 동일한 것처럼 느껴지지만 다음 차이점이 있다.
1. Commit Log : 각 트랜잭션의 상태 (COMMIT, ABORT, IN_PROGRESS, SUB_COMMIT)를 보유하며, 특히 Transaction Slot에는 없는 COMMIT/ABORT여부까지 알고 있다.
2. Transaction Slot : 오직 현재 진행중인 트랜잭션XID정보만을 보여준다. Commit/Abort여부는 알지 못한다.
...
- 예를 들어 Transaction Snapshot이 다음과 같다고 하자.
  ...
  100:104:100,102,103
  ```
   - 현재 진행중인 트랜잭션의 ID는 100번,102번,103번,104번이고, 이 트랜잭션의 데이터는 Invisible하다. 이 시점 102번 트랜잭션이 Commit으로 종료했다고 하더라도, 일단 Transaction Slot의 목록에 있다면 활성 트랜잭션으로 간주된다.
   - XMIN=100번이므로 100보다 작은 트랜잭션은 일단 Visible함.
   - XMAX=104번이므로 104이거나(현재진행형임) 더 큰 트랜잭션은 Invisible함.
...
- Read Committed
   ...
   | Session#2 (586) 커밋 | 584:590:584,588 | 2 | 586번 트랜잭션이 비활성상태 2번 조회됨 |
