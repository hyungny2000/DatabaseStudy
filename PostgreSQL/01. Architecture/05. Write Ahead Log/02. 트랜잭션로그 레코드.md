...
우선 `CREATE TABLE TEST1;` 한 줄만 분석해보자
```
rmgr: Storage len (rec/tot): 42/42, tx:496, lsn: 0/016E44B0, prev 0/016E4480, desc: CREATE base/16412/24609
```
...
- 이후에도 `CREATE TABLE`문 이하로 496번 트랜잭션(`tx : 496`) 내에서 작업된 이력이 수십 줄은 된다. 테이블 하나를 생성하고 유지하기 위해 글로벌 뷰에 INSERT하는 등의 작업이 이어진다.
...
rmgr: Transaction len (rec/tot):     34/    34, tx:        501, lsn: 0/016F9FC8, prev 0/016F9F90, desc: COMMIT 2025-02-20 23:19:01.365023 UTC
```
- 첫 번째 줄에서 테이블의 4번째 힙튜플에서 `KEY_UPDATED`신호와 함께 UPDATE되었다는 뜻이다. 이와 함께 `new off 7 xmax 0, blkref #0: rel 1663/16412/24609 blk 0`에서 볼 수 있듯, 0번 페이지의 7번째 힙튜플을 삽입한 것으로 보아, 7번째 튜플이 최신버전, 4번째 튜플은 DEAD TUPLE이 되었다는 것을 알 수 있다. 나머지 UPDATE구문도 비슷하다.
