...
rmgr: Transaction len (rec/tot):     34/    34, tx:        502, lsn: 0/016FA0B0, prev 0/016FA078, desc: ABORT 2025-02-20 23:19:14.422255 UTC
```
- 페이지 헤더의 lsn(pd_lsn) 값 0/16FA0B0이 트랜잭션로그의 ROLLBACK(ABORT)시점의 LSN과 일치한다. 가장 마지막에 실행된 트랜잭션의 LSN이 남는 것으로 보인다. commit/rollback이 안된 버퍼는 dirty가 아닌 pinned상태이므로 체크포인트를 해도 디스크에 flush되지 않는다. 따라서 commit/rollback상태에서도 버퍼의 pd_lsn에 LSN이 찍히는지 안찍히는지는 페이지만 보고는 알 수 없다.
