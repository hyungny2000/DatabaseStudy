...
| effective_cache_size (integer ) | 4GB | 시스템 메모리의 50% ~ 75% | | 단일 세션에서 사용할 수 있는 메모리캐시의 효율적인 크기의 추정치. OS의 공유메모리 설정에 대한 영향도가 없으며 커널 디스크 캐시를 예약하지도 않음.<br> 크면 클수록 인덱스 스캔을 하는 경향이 있다. |
