...
```
docker run -d -it --name postrm01 --hostname postrm01 --net host --ip 192.168.0.100 --privileged postgres12-test:latest /sbin/init
docker run -d -it --name postrm02 --hostname postrm02 --net host --ip 192.168.0.101 --privileged postgres12-test:latest /sbin/init
```
- mybridge 네트워크 내에선 ARP를 지원하지 않아 추후 VIP관리 구축이 어렵기 때문에, host네트워크로 세팅해야 함. (근데 고정IP 지정이 안됨)
