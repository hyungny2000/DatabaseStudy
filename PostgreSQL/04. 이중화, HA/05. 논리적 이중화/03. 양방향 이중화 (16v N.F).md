...
1. 예시
    ...
    owned by test1.col1;
    ```
   - `increment by n` : 시퀀스 번호 간의 간격을 노드 수에 일치시킨다. 예시에서 노드 수는 2 이므로 2로 설정한다.
   - `start with 10` : 시작값을 노드 별로 다르게 한다. 예를 들어 postlr01은 홀수, postlr02는 짝수를 채번하게 만든다면, postlr01은 5, postlr02는 6으로 설정한다. `increment by 2`에서 2씩 증가하게 되면 postlr01은 홀수만, postlr02는 짝수만 채번할 수 있다.
