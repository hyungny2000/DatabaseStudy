...
1. pg_wal디렉토리 disk full 장애
   ...
   - 별도 설정이 없다면 wal file이 점점 쌓이게 되고, pg_wal 디렉토리의 디스크 용량을 초과하여 DB Hang 장애가 발생할 수 있다. 물론 wal_keep_size나 wal_keep_segment 파라미터 설정이 안되어 있거나, 되어있어도 디스크 용량을 고려하지 않고 설정했을 때에는 발생이 가능하다.
