...
## 인덱스 생성시 고려사항
### 인덱스 스캔과 Sequential Scan(Table Full Scan) 비교해야 함.
1. 인덱스 스캔
   - Single Block I/O : 한 번에 한 블록씩만 Fetch 가능
   - Random Access : 인덱스 Leaf Block에서 테이블 페이지를 Fetch하는 접근을 말하는데, 테이블이 인덱스 컬럼 기준으로 얼마나 잘 정렬되어 있는지에 따라 성능이 크게 달라짐.
   - 손익 분기점 : **일반적으로 테이블 전체 건수의 5% 미만으로 조회시 인덱스가 유리**하다고 알려져 있다. (실제로는 더 적다...) 5%를 초과하면 Table Full Scan이 유리하다고 한다.
   - 선택 기준 : 전체 테이블 중에 1~2개 또는 수십 건 정도로 아주 적은 건수를 조회하고자 할 때에는 인덱스 사용을 고려한다.
2. Sequential Scan
   - Multi Block I/O : 한 번에 몇 십 블록씩 Fetch가 가능하여, 많은 블록을 조회할 때 Single Block I/O보다 I/O 성능이 더 좋다.
   - 손익 분기점 : 테이블 전체 건수의 5%이상, 또는 거의 대부분을 조회할 거라면 인덱스 스캔보다는 Sequential Scan이 더 낫다.

### 인덱스 선두컬럼을 잘 선정해야 함
1. 테이블에서 중복이 없거나 없는 컬럼을 선정해야 함
   - 중복이 많은 데이터는 인덱스 스캔 범위를 넓히며, 스캔 범위가 넓을수록 인덱스 스캔 성능은 현격히 떨어진다.
   - 중복이 없거나 적은 컬럼을 선두컬럼으로 지정하면 인덱스 스캔 범위를 좁힐 수 있다.
   - 예시 : 주민등록번호, 전화번호, 계좌번호 등
