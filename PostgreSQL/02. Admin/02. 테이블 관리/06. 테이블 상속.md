...
QUERY PLAN                                                                  |
----------------------------------------------------------------------------+
Append  (cost=0.28..26.80 rows=2 width=9)                                   |
  ->  Index Scan using test1_pkey on test1  (cost=0.28..8.29 rows=1 width=9)|
        Index Cond: (col1 = 1)                                              |
  ->  Seq Scan on test2  (cost=0.00..18.50 rows=1 width=9)                  |
        Filter: (col1 = 1)                                                  |
```
- 자기 테이블을 PK인덱스 Scan함 (비용 8.29)
- 자식 테이블을 Full Table Scan함 (비용 18.50)
- 총 비용 : 26.8
- **자식 테이블을 Full Scan하는 오버헤드**로 인해, 비상속 테이블 쿼리 실행시보다 비용이 배 이상 증가함.
...
## 결론
- 테이블 간의 상속관계를 형성하여 동일한 속성을 간편하게 공유하고 DB데이터를 상당 부분 절약할 수 있다.
- 상속 테이블의 쿼리 실행시 비용은 비상속 테이블보다 크다. 자식 테이블을 Full Scan하는 오버헤드 때문이다.
- 부모 테이블 변경 시 자식 테이블에도 테이블 Lock이 전파되므로, 광범위한 서비스장애를 유발한다.
